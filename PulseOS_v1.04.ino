// ---- Laser Sweep type (must be ABOVE Arduino auto-prototypes) ----
struct Laser {
  bool active = false;
...
struct RcCode {
  bool valid = false;
  uint16_t symCount = 0;
  rmt_symbol_word_t sym[RC_SYMBOL_MAX];
};
...
// Helper: trim string to fit width (simple, safe)
...
static void drawMenuSprite() {
  ...
  menuCanvas.println("NEON HUD INTERFACE");
...
// =======================================================
// LASER SWEEP (shorter segments + sweep fixed)
// =======================================================
...
static const int LS_SEG_MIN = 28;
static const int LS_SEG_MAX = 51;
...
static void gravityEnter() {   // KEEP HOOK NAME
  irToolsActive = false;
  speakerEnsureOn();
  lsReset();
}

static void gravityLoop() {    // KEEP HOOK NAME
  if (btnBHoldExit()) {
    uiBeep(BEEP_BACK);
    enterApp(APP_PLAY);
    return;
...
// =======================================================
// PARTICLE LAB (unchanged from your provided code)
// =======================================================
...
static void particleLoop() {
  ...
  appCanvas.setCursor(8, 20);
  appCanvas.setTextColor(subC);
  appCanvas.printf("A: Reset   Stored: %d/%d", pl_got, pl_need);
...
// =======================================================
// IR TOOLS (unchanged from your provided code)
// =======================================================
...
static const uint8_t RC_STORE_VER = 2;
...
static void rcToast(const char* msg, uint32_t ms = 1200) {
...
static void rmtStartReceive() {
  ...
  (void)rmt_receive(rx_chan, rx_raw_symbols, sizeof(rx_raw_symbols), &cfg);
}
...
static void rmtSetupTx() {
  ...
  rmt_carrier_config_t carrier_cfg = {
    .frequency_hz = (uint32_t)IR_CARRIER_FREQ_HZ,
...
static void rmtSendSymbols(const rmt_symbol_word_t *sym, size_t count) {
  if (tx_chan == NULL || copy_encoder == NULL) return;
  if (count == 0) return;

  rmt_transmit_config_t cfg = { .loop_count = 0 };
  rmt_transmit(tx_chan, copy_encoder, sym, count * sizeof(rmt_symbol_word_t), &cfg);
  rmt_tx_wait_all_done(tx_chan, 1000);
}

static void rcMakeKey(char* out, size_t outLen, int p, int f) {
...
static void rcSaveOne(int p, int f) {
  RcCode &c = rcDB[p][f];
  char key[8];
  rcMakeKey(key, sizeof(key), p, f);

  if (!c.valid || c.symCount < 2) {
    rcPrefs.remove(key);
    return;
  }

  const size_t bytes = 2 + c.symCount * sizeof(rmt_symbol_word_t);
  if (bytes > (2 + RC_SYMBOL_MAX * sizeof(rmt_symbol_word_t))) return;

  uint8_t buf[2 + RC_SYMBOL_MAX * sizeof(rmt_symbol_word_t)];
  buf[0] = (uint8_t)(c.symCount & 0xFF);
  buf[1] = (uint8_t)((c.symCount >> 8) & 0xFF);
  memcpy(&buf[2], c.sym, c.symCount * sizeof(rmt_symbol_word_t));

  rcPrefs.putBytes(key, buf, bytes);
}
...
static void rcLoadOne(int p, int f) {
  ...
  size_t n = rcPrefs.getBytesLength(key);
  if (n < 2 || n > (2 + RC_SYMBOL_MAX * sizeof(rmt_symbol_word_t))) {
    c.valid = false; c.symCount = 0;
    return;
  }

  uint8_t buf[2 + RC_SYMBOL_MAX * sizeof(rmt_symbol_word_t)];
  size_t got = rcPrefs.getBytes(key, buf, n);
  if (got != n) { c.valid = false; c.symCount = 0; return; }

  uint16_t count = (uint16_t)buf[0] | ((uint16_t)buf[1] << 8);
  if (count < 2 || count > RC_SYMBOL_MAX) { c.valid = false; c.symCount = 0; return; }

  size_t need = 2 + count * sizeof(rmt_symbol_word_t);
  if (n != need) { c.valid = false; c.symCount = 0; return; }

  memcpy(c.sym, &buf[2], count * sizeof(rmt_symbol_word_t));
  c.symCount = count;
  c.valid = true;
}
...
static void rcLoadAll() {
  ...
  for (int p = 0; p < RC_PROFILES; p++) {
    for (int f = 0; f < RC_FN_COUNT; f++) rcLoadOne(p, f);
  }
}

static bool rcStoreFromRx(RcCode &dst) {
  if (rx_symbol_num < 2) return false;

  size_t n = rx_symbol_num;
  if (n > RC_SYMBOL_MAX - 1) n = RC_SYMBOL_MAX - 1;

  for (size_t i = 0; i < n; i++) {
    rmt_symbol_word_t s = rx_raw_symbols[i];
    s.level0 = !s.level0;
    s.level1 = !s.level1;
    dst.sym[i] = s;
  }
  ...
  gap.level0 = 0;
  gap.duration0 = 30000; // 30ms
  gap.level1 = 0;
  gap.duration1 = 0;
  dst.sym[n] = gap;

  dst.symCount = (uint16_t)(n + 1);
  dst.valid = true;
  return true;
}
...
static void rcSendSelected() {
  ...
  if (!c.valid || c.symCount < 2) return;

  for (int i = 0; i < 3; i++) {
    rmtSendSymbols(c.sym, c.symCount);
    delay(40);
  }
}
...
static void irLoop() {
  ...
  if (!chordActive) {
    ...
    if (aPress && !rcLearning) {
      RcCode &c = rcDB[rcProfile][rcSelFn];
      if (c.valid) { rcSendSelected(); uiBeepIR(BEEP_OK); rcToast("Sent"); }
      else { uiBeepIR(BEEP_BACK); rcToast("No code stored"); }
    }
  }
  ...
  if (rx_done) {
    ...
    if (rcLearning) {
      if (rx_seq <= rcLearnArmSeq || millis() < rcLearnIgnoreUntilMs) {
        ...
        if (rcStoreFromRx(dst)) {
          rcSaveOne(rcProfile, rcSelFn);
          uiBeepIR(BEEP_OK);
          rcToast("Learned + saved");
        } else {
          uiBeepIR(BEEP_BACK);
          rcToast("Learn failed");
        }
        rcLearning = false;
        rmtStartReceive();
      }
  ...
  appCanvas.setTextColor(subC);
  appCanvas.setCursor(8, 8);
  appCanvas.println("IR TOOLS  (hold B to exit)");
  ...
  // Status line
  appCanvas.setTextColor(subC);
  appCanvas.setCursor(12, 86);
  if (rcLearning) {
    uint32_t sec = (rcLearnUntilMs > millis()) ? (uint32_t)((rcLearnUntilMs - millis()) / 1000) : 0;
    appCanvas.setTextColor(pink);
    appCanvas.printf("Learning: %lus", (unsigned long)sec);
  } else {
    appCanvas.printf("Last RX: %s", (irLastHitMs ? "HIT" : "None"));
  }

  appCanvas.setTextColor(subC);
  appCanvas.setCursor(12, 102);
  if (irLastHitMs) appCanvas.printf("sym:%u  age:%lus", irLastSymCount, (unsigned long)((millis()-irLastHitMs)/1000));

  // Footer
  const int footerY = 114;
  const int footerH = H - footerY;
  appCanvas.fillRect(0, footerY, W, footerH, TFT_BLACK);
