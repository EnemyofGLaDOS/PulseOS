...
static constexpr uint16_t RC_RAW_STORE_MAX = 180;

struct RcCode {
  bool valid = false;
  bool isNEC = false;          // true = NEC, false = RAW
  uint16_t address = 0;
  uint8_t command = 0;
  uint8_t repeats = 0;

  uint16_t rawCount = 0;
  rmt_symbol_word_t raw[RC_RAW_STORE_MAX];
};
...
static App apps[APP_COUNT] = {
  ...
  { "LASER SWEEP", gravityEnter,    gravityLoop },
...
static uint32_t tiltBestCs = 0;
static uint32_t tiltLastCs = 0;
static uint32_t tiltLevelStartMs = 0;
static uint32_t laserBestMs = 0;
static uint32_t particleBestMs = 0;
static uint32_t particleStartMs = 0;
...
static bool    ui_flipRotation = false;
static uint8_t ui_theme = 0;
...
static void lsSpawnOne(uint32_t now) {
  ...
  float base = random(55, 95) / 100.0f;
...
static const uint8_t RC_STORE_VER = 4;
...
static void rcZeroCode(RcCode &c) {
  c.valid = false;
  c.isNEC = false;
  c.address = 0;
  c.command = 0;
...
static void rcSaveOne(int p, int f) {
  ...
  rcPrefs.putUShort((String(key) + "l").c_str(), c.rawCount);

  if (c.rawCount > 0) {
    rcPrefs.putBytes((String(key) + "b").c_str(), c.raw, c.rawCount * sizeof(rmt_symbol_word_t));
  } else {
    rcPrefs.remove((String(key) + "b").c_str());
  }
}
...
static void rcLoadOne(int p, int f) {
  ...
  c.rawCount = rcPrefs.getUShort((String(key) + "l").c_str(), 0);

  if (c.rawCount > RC_RAW_STORE_MAX) c.rawCount = 0;

  if (c.rawCount > 0) {
    size_t want = c.rawCount * sizeof(rmt_symbol_word_t);
    size_t got = rcPrefs.getBytes((String(key) + "b").c_str(), c.raw, want);
    if (got != want) {
      c.rawCount = 0;
      memset(c.raw, 0, sizeof(c.raw));
    }
...
static size_t rcBuildTxFromStoredRaw(const RcCode& c, rmt_symbol_word_t *out, size_t maxOut) {
  if (!c.valid || c.isNEC || c.rawCount == 0 || !out || maxOut == 0) return 0;

  size_t outCount = 0;

  auto pushSym = [&](uint32_t markUs, uint32_t spaceUs) -> bool {
    if (markUs == 0 && spaceUs == 0) return true;
    if (outCount >= maxOut) return false;

    out[outCount].level0 = 1;   // mark = carrier ON
    out[outCount].duration0 = markUs;
    out[outCount].level1 = 0;   // space = carrier OFF
    out[outCount].duration1 = spaceUs;
    outCount++;
    return true;
  };

  for (uint16_t i = 0; i < c.rawCount; i++) {
    const rmt_symbol_word_t &s = c.raw[i];

    uint32_t markUs = 0;
    uint32_t spaceUs = 0;

    // RX data from demodulated IR receiver:
    // LOW = mark, HIGH = space
    if (s.level0 == 0) markUs += s.duration0;
    else               spaceUs += s.duration0;

    if (s.level1 == 0) markUs += s.duration1;
    else               spaceUs += s.duration1;

    if (markUs == 0 && spaceUs == 0) continue;

    if (spaceUs > 30000) spaceUs = 30000;

    if (!pushSym(markUs, spaceUs)) break;
  }
...
static void rcSendSelected() {
  ...
  if (!c.valid) {
    rcToast("No code stored");
    return;
  }

  if (tx_chan == NULL || copy_encoder == NULL) {
    rcToast("TX not ready");
    return;
  }
  ...
  if (c.isNEC) {
    ...
    rmt_symbol_word_t txRaw[RC_RAW_STORE_MAX];
    size_t count = rcBuildTxFromStoredRaw(c, txRaw, RC_RAW_STORE_MAX);
  ...
  if (err != ESP_OK) {
    ...
    rcToast(c.isNEC ? "Sent NEC" : "Sent RAW");
  }
}
...
static void irLoop() {
  ...
  if (rx_done) {
    ...
    if (rcLearning) {
      if (rx_seq <= rcLearnArmSeq || millis() < rcLearnIgnoreUntilMs) {
        ...
        } else if (rx_symbol_num >= 6) {
          dst.valid = true;
          dst.isNEC = false;
          dst.rawCount = (rx_symbol_num > RC_RAW_STORE_MAX) ? RC_RAW_STORE_MAX : (uint16_t)rx_symbol_num;
          memcpy(dst.raw, rx_raw_symbols, dst.rawCount * sizeof(rmt_symbol_word_t));
          rcSaveOne(rcProfile, rcSelFn);
          uiBeepIR(BEEP_OK);
          rcToast(dst.rawCount < rx_symbol_num ? "RAW saved (trimmed)" : "RAW learned + saved");
        } else {
          rcZeroCode(dst);
          uiBeepIR(BEEP_BACK);
          rcToast("Capture too short");
        }
  ...
  appCanvas.setTextColor(subC);
  appCanvas.setCursor(8, 8);
  appCanvas.println("IR TOOLS NEC+RAW  (hold B to exit)");
  ...
  if (!c.valid) {
    appCanvas.printf("Stored: NO");
  } else if (c.isNEC) {
    appCanvas.printf("Stored: NEC");
  } else {
    appCanvas.printf("Stored: RAW");
  }
  ...
  if (c.valid && c.isNEC) {
    ...
    appCanvas.printf("Raw symbols: %u", c.rawCount);
  } else {
  ...
  if (millis() < rcToastUntil && rcToastMsg[0]) {
    ...
    appCanvas.println("HoldA:Learn NEC/RAW  A+B:Profile");
  }
}
